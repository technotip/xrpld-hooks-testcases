...
int64_t hook(int64_t reserved ) {
    ...
    {
	uint8_t hash[32];
	int64_t result = ledger_last_hash(SBUF(hash));	    

        if(result != 32)
                rollback(SBUF("Testcase1: Couldn't fetch the last ledger hash."), 1);
    ...
    {
        uint8_t hash[32];
        int64_t result = ledger_last_hash(hash, 99999999); //2nd parameter is invalid and OUT_OF_BOUNDS.
