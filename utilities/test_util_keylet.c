...
uint8_t oracle_hi[20] = { // require('ripple-address-codec').decodeAccountID('r9PfV3sQpKLWxccdg3HL2FXKxGW2orAcLE')
    ...
    int64_t txn[32]; // Will be used for testcase 6.
	...
	int64_t slot_no = slot_set(SBUF(keylet), 0);
	slot(SBUF(txn), 1); // This is to be used in testcase 6.
	trace(SBUF("txn value check"), SBUF(txn), 1);
	trace_num(SBUF("Testcase5: slot_no"), slot_no);
        ASSERT( result == 34 ); // Expected: 34. 34 bytes written into butter referenced by keylet.
    }    
...
/*
	    ...
	    int64_t result = util_keylet(SBUF(keylet), KEYLET_CHILD, SBUF(txn), 0, 0, 0, 0);
*/
//	    if(result != 34)
//  rollback(SBUF("Testcase6: Internal error, count not generate keylet."), 11);


/*
	    trace(SBUF("Testcase6: KEYLET_CHILD"), SBUF(keylet), 1);
	    
	    trace_num(SBUF("Testcase6 Result "), result);
*/
	uint8_t keylet[34];
    //    int64_t slot_no = slot_set(SBUF(keylet), 0);	    
	   
   //     trace_num(SBUF("6. Slot no"), slot_no);
 
	trace(SBUF("6. keylet valie "), SBUF(keylet), 1);

	   // trace_num(SBUF("Testcase6: slot_no"), slot_no);
